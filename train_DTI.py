...
def step(model, batch, device=None):
    ...
    return pred.view(-1), label.view(-1)  # Ensure both are flattened
...
def contrastive_step(model, batch, device=None):
    ...
    positive_transformed = model.drug_input_transform(positive.to(device))
    negative_transformed = model.drug_input_transform(negative.to(device))

    anchor_projection = model.target_projector(anchor.to(device))
    positive_projection = model.drug_projector(positive_transformed)
    negative_projection = model.drug_projector(negative_transformed)
...
def contrastive_step(model, batch, device=None):
    ...
    print(f"Original positive shape: {positive.shape}")
    print(f"Original negative shape: {negative.shape}")
    # 打印 drug_input_transform 的权重形状
    print(f"drug_input_transform weight shape: {model.drug_input_transform.weight.shape}")

    # Check and ensure the shape of positive and negative matches the input requirements of the model
    # If necessary, reshape the tensors to ensure compatibility with the linear layer
    positive_transformed = model.drug_input_transform(positive.to(device).view(positive.size(0), -1))
    negative_transformed = model.drug_input_transform(negative.to(device).view(negative.size(0), -1))

    # 打印调整后的形状以确认
    print(f"Transformed positive shape: {positive_transformed.shape}")
    print(f"Transformed negative shape: {negative_transformed.shape}")
    ...
    # 打印投影后的形状
    print(f"Anchor projection shape: {anchor_projection.shape}")
    print(f"Positive projection shape: {positive_projection.shape}")
    print(f"Negative projection shape: {negative_projection.shape}")

    return anchor_projection, positive_projection, negative_projection
'''


# 定义一个线性降维层，将 768 维输入转换为 300 维
class ModelWithDimReduction(torch.nn.Module):
    def __init__(self):
        super(ModelWithDimReduction, self).__init__()
        self.dim_reduction = torch.nn.Linear(768, 300)  # 将 768 降维到 300
        self.drug_input_transform = torch.nn.Linear(300, 1024)  # 现有的层
        self.target_projector = torch.nn.Linear(768, 512)  # 假设 anchor 的维度是 768
        self.drug_projector = torch.nn.Linear(1024, 512)  # 假设输出维度为 512

    def forward(self, anchor, positive, negative):
        # 对 positive 和 negative 进行降维
        positive_reduced = self.dim_reduction(positive)
        negative_reduced = self.dim_reduction(negative)

        # 将降维后的数据输入到 drug_input_transform 层
        positive_transformed = self.drug_input_transform(positive_reduced)
        negative_transformed = self.drug_input_transform(negative_reduced)

        anchor_projection = self.target_projector(anchor)
        positive_projection = self.drug_projector(positive_transformed)
        negative_projection = self.drug_projector(negative_transformed)
...
def contrastive_step(model, batch, device=None):
    ...
    # 使用降维层对 positive 和 negative 进行降维
    positive_transformed = model.dim_reduction(positive.to(device))
    negative_transformed = model.dim_reduction(negative.to(device))

    # 之后进入 drug_input_transform
    positive_transformed = model.drug_input_transform(positive_transformed)
    negative_transformed = model.drug_input_transform(negative_transformed)
...
def main():
    ...
    max_metric = 0
    torch.save(model.state_dict(), 'initial_model_state.pth')  # 保存初始模型状态
        ...
        if epo % config.every_n_val == 0:
            with torch.set_grad_enabled(False):
                ...
                if val_results[config.watch_metric] > max_metric:
                    logg.debug(
                        f"Validation {config.watch_metric} {val_results[config.watch_metric]:8f} > previous max {max_metric:8f}"
                    )
                    torch.save(model.state_dict(), 'best_model_state.pth')  # 保存最佳模型状态
    ...
    try:
        with torch.set_grad_enabled(False):
            ...
            model_max.eval()
            model_max.load_state_dict(torch.load('best_model_state.pth'))  # 加载最佳模型状态
