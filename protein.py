...
# 明确设置项目路径
project_dir = '/data/hanyuhang/ssw_lab/qbx/drug/conban1/src'
# 添加项目目录到 sys.path
sys.path.append(project_dir)

# 打印 sys.path 确认项目目录已添加
print(sys.path)
...
class ProtBertFeaturizer(Featurizer):
    def __init__(self, save_dir: Path = Path().absolute(), per_tok=False):
        ...
        local_model_path = "/home/qbx/drug/ConPLex_dev/bert"
        ...
        self._protbert_tokenizer = AutoTokenizer.from_pretrained(local_model_path, do_lower_case=False, cache_dir=f"{MODEL_CACHE_DIR}/huggingface/transformers")
        self._protbert_model = AutoModel.from_pretrained(local_model_path, cache_dir=f"{MODEL_CACHE_DIR}/huggingface/transformers")
        self._protbert_feat = pipeline("feature-extraction", model=self._protbert_model, tokenizer=self._protbert_tokenizer)
...
class ProtBertFeaturizer(Featurizer):
    def __init__(self, save_dir: Path = Path().absolute(), per_tok=False):
        ...
        local_model_path = "/data/hanyuhang/ssw_lab/qbx/drug/conban1/bert"
        ...
        self._protbert_tokenizer = AutoTokenizer.from_pretrained(local_model_path, do_lower_case=False, cache_dir=f"{MODEL_CACHE_DIR}/huggingface/transformers")
        self._protbert_model = AutoModel.from_pretrained(local_model_path, cache_dir=f"{MODEL_CACHE_DIR}/huggingface/transformers")
        self._protbert_feat = pipeline("feature-extraction", model=self._protbert_model, tokenizer=self._protbert_tokenizer, device=self._device.index if self._device.type == "cuda" else -1)
