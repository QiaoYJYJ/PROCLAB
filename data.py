...
def get_task_dir(task_name: str):
    ...
    task_paths = {
        ...
        "davis": "./dataset/DAVIS",
        "biobin": "./dataset/bio-bin",
        "bioda": "./dataset/bio-da",
        "quan": "./dataset/quan",
        "dbda": "./dataset/dbda",
        "dti_dg": "./dataset/TDC",
        "dude": "./dataset/DUDe",
...
def make_contrastive(
    ...
    print(f"生成的对比样本数量: {len(contrastive)}")
    return contrastive
...
class DUDEDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: T.Optional[str] = None):
        ...
        self.df_full = pd.read_csv(
            self._data_dir / Path("full_train.tsv"), **self._csv_kwargs
        )
        ...
        # 打印数据集长度
        print(f"训练数据集长度: {len(self.df_train)}")
        print(f"测试数据集长度: {len(self.df_test)}")
        ...
        if stage == "fit" or stage is None:
            ...
            print(f"数据集长度: {len(self.data_train)}")

    def train_dataloader(self):
        print(f"训练样本数量: {len(self.data_train)}")
        return DataLoader(self.data_train, **self._loader_kwargs)
